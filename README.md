...
Moreover, HATEOAS makes discoverability possible. Any client who has never encountered any previous documentation can start at the starting Discovery endpoint (/api/v1) and follow the entire API by navigating the hypermedia links in each response, just as they would a web site. This helps to minimise the versioning between client and server implementations, as the server is in charge of dynamic navigation and not the client using old and stale documentation that might not be a true mirror of the actual API structure.
...
Technically, the difference between a Sub-Resource Locator and a conventional resource method is that the locator method does not have an HTTP verb annotation like an annotation of either type: @GET or an annotation of type: @POST. This lack indicates to the JAX-RS runtime that this method does not process the request itself, but rather returns an object instance - in our case SensorReadingResource - on which the runtime then delegates all further routing and HTTP method resolution. This implies that the JAX-RS engine will dynamically examine the returned class on-the-fly and compare the remaining path segments and the HTTP verb to enable the routing logic of multiple focused classes to be separated in a clean manner instead of a single monolithic controller.
...
As an example, a stack trace with the output of `org.glassfish.jersey.server-2.41` will tell the attacker directly what version and what framework they are dealing with, which can be compared to published databases of CVEs (Common Vulnerabilities and Exposures) to determine which known exploits apply to that version. Likewise, uncovered package names include `com.smartcampus.repository.RoomRepository` disclose the architecture and the structure of the classes, enabling an attacker to develop specific injection payloads. Path names of file systems that are visible in traces can give away server directory structures and exception messages can unintentionally disclose fragments of database queries or connection strings also. This intelligence enables accurate exploitation patterns. Our application prevents this precisely by the `GenericExceptionMapper` mapping `Throwable.class`, which entirely hides internal leakage and returns in a graceful way only an opaque HTTP `500 Internal Server Error`.
